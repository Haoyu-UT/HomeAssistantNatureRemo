...
* movement sensor
* electricity meter
* adjustable polling rates for sensors and appliance state synchronization
...
* Light and air conditioner operations through Nature Home app can be synchronized, operations through physical remote control can be synchronized if you have latest Nature Remo hardware
...
### Manual installation
Copy the folder `custom_components/nature_remo` into `<config directory>/custom_components/` on your device. If you do not have access to the `<config directory>`, you can install the `Samba share` add-on and access with SAMBA.
...
**If your light only toggles, on/off is for manually fixing wrong state.** You are asked to designate a signal for on and a signal for off. They can be same or different, and if you assigned the same signal for on and off it means your light can only toggle. On and off will first overwrite its current state and send the designated signal.
